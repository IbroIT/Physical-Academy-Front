...
const EbilimLogin = () => {
  ...
  const startCounters = () => {
    // Safely get stats with array checking
    const statsData = Array.isArray(
      t("students.ebilim.stats", { returnObjects: true })
    )
      ? t("students.ebilim.stats", { returnObjects: true })
      : [];

    // Safely parse numbers from the stats values with error handling
    const targetValues = statsData.map((stat) => {
      if (!stat || typeof stat.value !== "string") return 0;
      try {
        // Remove all non-digit characters and parse as integer
        return parseInt(stat.value.replace(/\D/g, "")) || 0;
      } catch (error) {
        console.error("Error parsing stat value:", error);
        return 0;
      }
    });
    ...
    const counterInterval = setInterval(() => {
      ...
      setCounterValues((prev) => {
        // Ensure prev is an array and has the right length
        const prevValues = Array.isArray(prev) ? prev : [0, 0, 0, 0];

        return prevValues.map((value, index) => {
          // Check if the index exists in stepValues array
          if (index < stepValues.length && currentStep <= steps) {
            const stepValue = stepValues[index] || 0;
            const targetValue = targetValues[index] || 0;
            return Math.min(value + stepValue, targetValue);
          }
          return value;
        });
      });
  ...
  // Safely get translation data with fallback for arrays
  ...
  const quickLinks = Array.isArray(
    t("students.ebilim.quickLinks", { returnObjects: true })
  )
    ? t("students.ebilim.quickLinks", { returnObjects: true })
    : [];
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0">
        ...
        <div className="absolute bottom-1/3 left-1/4 text-3xl opacity-5">
          📚
        </div>
      ...
      <div className="relative z-10">
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="text-blue-100 text-lg leading-relaxed">
                  {t(
                    "students.ebilim.description",
                    "Доступ к образовательным ресурсам и онлайн-курсам"
                  )}
                </p>
              </div>
            </motion.div>
                ...
                >
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {Array.isArray(quickLinks) && quickLinks.length > 0 ? (
                      ...
                      <div className="col-span-2 md:col-span-4 text-center p-4 bg-white/5 rounded-xl backdrop-blur-sm">
                        <p className="text-blue-200">
                          {t(
                            "students.ebilim.noQuickLinks",
                            "Quick links will be available soon."
                          )}
                        </p>
                      </div>
                    )}
        ...
        >
          {Array.isArray(stats) && stats.length > 0 ? (
            stats.map((stat, index) => (
              ...
              >
                <div className="text-2xl lg:text-3xl font-bold mb-1 font-mono bg-gradient-to-r from-blue-400 to-emerald-400 bg-clip-text text-transparent">
                  {stat?.value?.includes?.("%")
        ...
        >
          <div className="flex items-center">
            ...
            <span>
              {t("students.ebilim.status.online", "Система работает стабильно")}
            </span>
          </div>
          <div className="flex items-center">
            ...
            <span>
              {t(
                "students.ebilim.status.updates",
          ...
          <div className="flex items-center">
            ...
            <span>
              {t(
                "students.ebilim.status.support",
